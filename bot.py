...
def menu_inline_keyboard(
    *,
    ...
    # Tea item selector: show as inline buttons and reveal description on tap.
    if active == "menu_tea":
        def _tea_btn(title: str, key: str) -> InlineKeyboardButton:
            cb = f"menu_tea_item:{key}"
            if tea_active and tea_active == key:
                return _StyledInlineButton(text=title, callback_data=cb, style="primary")  # type: ignore[return-value]
            return InlineKeyboardButton(text=title, callback_data=cb)
...
def handle_menu_sections(call: telebot.types.CallbackQuery) -> None:
    ...
    TEA_ITEMS: dict[str, tuple[str, str]] = {
        "sencha": (
            "Сенча (Шу Сян Люй)",
            "Листовой чай, который скручивается в тонкие полоски-иголки, заваривается водой 80-85°C и обладает антиоксидантными свойствами, тонизирует, богат витаминами.",
        ),
        "milky_oolong": (
            "Молочный улун",
            "Популярный китайский полуферментированный чай (15-50% ферментации) с выраженным сливочно-карамельным ароматом и мягким вкусом. Заваривается при 85-95°C, содержит витамины, помогает пищеварению и расслабляет.",
        ),
        "dian_hong": (
            "Дянь хун маофен",
            "Высококачественный китайский красный чай из провинции Юньнань, сочетающий в себе название региона («Дянь») и метод обработки («Маофэн» - «ворсистые пики»), означающий сбор почки и двух верхних листочков; он славится насыщенным, сладковатым вкусом с нотами мёда, сухофруктов, выпечки, ярким медовым ароматом и золотисто-красным настоем, обладая при этом согревающим и бодрящим эффектом.",
        ),
        "puer_shu": (
            "Пуэр шу",
            "Ферментированный китайский чай из Юньнаня с ускоренным процессом «искусственного состаривания». Он отличается очень тёмным, густым настоем, древесно-шоколадным вкусом, землистыми нотками и сладким послевкусием. Обладает мощным тонизирующим эффектом, улучшает пищеварение и способствует концентрации.",
        ),
        "tieguanyin": (
            "Улун те гуань инь",
            "Знаменитый слабоферментированный китайский улун из провинции Фуцзянь, занимающий промежуточное место между зелёными и красными чаями. Характеризуется плотно скрученными листьями, свежим цветочно-медовым ароматом, густым зелёно-золотистым настоем и длительным сладким послевкусием. Обладает тонизирующим эффектом.",
        ),
    }

    def _tea_base_text() -> str:
        return (
            "КЛАССИЧЕСКИЙ ЧАЙ\n"
            "600мл / 320₽\n"
            "Ассам\n"
    ...
    def _text(cb: str, *, tea: str | None, show_drinks_rules: bool) -> str:
        ...
        if cb == "menu_tea":
            base = _tea_base_text()
            if tea and tea in TEA_ITEMS:
                title, desc = TEA_ITEMS[tea]
                return f"{base}\n\n<b>{title}</b>\n{desc}"
            return base
        if cb == "menu_food":
